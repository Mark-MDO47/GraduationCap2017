...
//     major kudos to Fr. Robert Ballecer and Bryan Burnett for that steampunk goggles project! 
//
...
#define NUM_CIRCLES 1     // README for testing/debugging just one ring, or if need multiple Arduinos might go there
#define NUM_LEDS_PER_CIRCLE 93
#define NUM_LEDS (NUM_CIRCLES * NUM_LEDS_PER_CIRCLE)
#define WHICH_CIRCLE 0 // if we have multiple Arduinos, this will identify them
...
int leds_per_ring_qrtr[NUM_RINGS_PER_CIRCLE] = { 8, 6, 4, 3, 2, 1 }; // save time of doing divide by 4 over and over
// direction per ring: 1 = clockwise, -1 = counterclockwise
int direc_per_ring[NUM_RINGS_PER_CIRCLE] = { 1, -1, 1, -1, 1, 1 };

// offset quarters per circle
//    direction: pos = clockwise, neg = counterclockwise
//    count: how many quarter-circles to rotate
int offset_qrtrs_per_circle[4] = { 0, 1, 2, 3 }; 
...
void loop() {
  ...
  if (0 == tmp_DEBUG) {
    for (tmp_DEBUG = 0; tmp_DEBUG < NUM_LEDS_PER_CIRCLE; tmp_DEBUG++) {
      // tmp_DEBUG2 = map_circle_led_num(tmp_DEBUG, WHICH_CIRCLE);
      tmp_DEBUG2 = map_circle_led_num(tmp_DEBUG, 1);
...
// maps a logical LED num from 0-92 into a physical LED num in a particular LED circle
//   takes into account that circles have their own orientation (see qrtr calculations)
//   takes into account rings go clockwise vs counterclockwise
//   physical LED num goes clockwise on outer ring, then clockwise next ring, etc.
//   led_display is organized in physical LED order
int map_circle_led_num(int logical_led_num, int which_circle) {
  int tot_leds_so_far = 0; // will be how many LEDs to get to our ring
  int tmp_led_num = logical_led_num; // will be LED within the ring
  for (int ring = 0; ring < NUM_RINGS_PER_CIRCLE-1; ring++) {
    if (tmp_led_num < leds_per_ring[ring]) {
      if (direc_per_ring[ring] > 0) return((tmp_led_num + leds_per_ring_qrtr[ring]*offset_qrtrs_per_circle[which_circle])%leds_per_ring[which_circle] + tot_leds_so_far);
      else return((leds_per_ring[ring]-tmp_led_num-1 + leds_per_ring_qrtr[ring]*offset_qrtrs_per_circle[which_circle])%leds_per_ring[which_circle] + tot_leds_so_far);
...
//      else return(tot_leds_so_far+leds_per_ring[ring] - logical_led_num  + tot_leds_so_far - 1);
    ...
    tmp_led_num -= leds_per_ring[ring];
  } // end for each ring
  return(logical_led_num); // here for LED 92 - no direction
} // end map_circle_led_num()
