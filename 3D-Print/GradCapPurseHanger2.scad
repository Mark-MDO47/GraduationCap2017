...
/*
    -----------------------------------------------------------
      ...
      roundCornersCube(x,y,z,r) Where: (rounding done x and x corners)
         ...
         - r = Rounding radius
      Example: roundCornersCube(10,10,2,1);
     *Some times it's needed to use F6 to see good results!
 	 -----------------------------------------------------------

NOTE: puts center at (0,0,0)
...
plate_totl_height =  8;   // mm including width of plate, and everything
...
btn_w_clearance       = 0.2;   // mm clearance width
btn_h_clearance       = 0.2;   // mm clearance width
btn_w_outside         = 6.5; // mm outside dimension
btn_h_outside         = 4.5; // mm outside dimension
...
module top(totalheight, platelength, platewidth, plateslotheight) {
    difference() {
        union() {
            ...
            // this puts the slots on the bottom of the plate
        ...
        union() {
            ...
            translate([leds_length_offset-(leds_cnctr_long-leds_cnctr_justone)/2,leds_width_offset-(leds_rect_skinny-leds_cnctr_skinny)/2,0.0*totalheight]) roundCornersCube(leds_cnctr_justone+1,leds_cnctr_skinny+1,0.5*totalheight,1);
        }
    }
}   // end module top()
...
module box(totalheight, long, skinny, width) {
    difference() {
        ...
        union() {
            ...
            translate([long/2,skinny/5,2*width+btn_h_clearance+(usb_h_outside+btn_h_clearance)/2]) cube(size = [8*wall_width,usb_w_outside+btn_w_clearance,usb_h_outside+btn_h_clearance], center = true);
...
top(totalheight = 14, plateslotheight = 3, platelength=120, platewidth=40);
...
// translate([0,0,4.0/2.0]) roundCornersCube(10,20,4,1);
